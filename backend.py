...
def init_db():
    ...
    ВАЖНО:
    база больше НЕ удаляется при каждом запуске сервера.
    """
    ...
    # Создаём демо-пользователей ТОЛЬКО если БД пустая
    ...
    token = jwt.encode(
        {
            ...
            ) + datetime.timedelta(days=TOKEN_DAYS)
        },
        SECRET_KEY,
        algorithm="HS256"
    )
...
def send_message():
    ...
    # Отправляем отправителю тоже,
    # чтобы можно было обновлять интерфейс
    socketio.emit(
        "message_sent",
        message,
...
def get_messages(username):
    ...
    # Всё полученное от партнёра становится прочитанным
    conn.execute("""
        UPDATE messages
        SET read = 1
        WHERE from_user = ?
...
def get_mood(username):
    ...
    # Пользователь может смотреть только своё состояние
...
def recover_password():
    ...
    # Пока не подключаем настоящую почту.
